...
public class Pessoa implements Serializable {
    ...
    // --- ALTERAÇÃO 1: Removido @NotNull ---
    // A validação de padrão continua ativa APENAS SE houver valor. Se for NULL, passa.
    @Pattern(
            regexp = "^(\\d{10}|\\d{9}-\\d{1,2}|\\d{3}\\.\\d{3}\\.\\d{3}-\\d{1,2})$",
            message = "Formato de identidade inválido. Aceita: 10 dígitos, 000.000.000-00 ou 000000000-00"
    )
    @Column(unique = true) // Cuidado: No Postgres, unique permite múltiplos NULLs, então ok.
    private String identidade;

    @NotNull // Nome continua obrigatório
    @Column
    private String nome;

    // --- ALTERAÇÃO 2: Removido @NotNull ---
    @Column
    private String nomeGuerra;

    @NotNull // Posto continua obrigatório
    @Column
    private PostoGraduacao postoGraduacao;
    ...
    // === RELACIONAMENTOS ===
    @NotNull // Assessoria/OM continua obrigatória
    @ManyToOne
    @JoinColumn(name = "assessoria_id")
    private Assessoria assessoria;
    ...
    // --- ALTERAÇÃO 4: Removido @NotNull ---
    @Column
    private String caminho;

    // --- ALTERAÇÃO 5: Removido @NotNull ---
    @Column(name="dt_nascimento")
    private LocalDate dtNascimento;
    ...
    public Users parseUsers() {
        Users users2 = new Users();
        // Cuidado aqui: Se a identidade for NULL, isso pode dar erro ao tentar logar.
        // Mas como é usuário externo, talvez nem deva ter login.
        if (identidade != null) {
            users2.setName(identidade);
            users2.setPass(EncodingSHA256.encodingBase64(identidade));
        } else {
            // Define um nome temporário ou ignora login para externos
            users2.setName("EXTERNO_" + id);
        }
        users2.addRole(TipoAcesso.USUARIO);
        return users2;
    }
