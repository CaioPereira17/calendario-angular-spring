...
//   // handleDateClick(event: any) {
//   //   this.videoConferencia = {id:0, pessoa:{_id:'', identidade:'', users:'', tipoAcesso:'', nome:'', nomeGuerra:'', postoGraduacao:'', acesso:'', dataUltimaPromocao:0, assessoria:{_id:'', sigla:'', descricao:'', interna:true, ordem:0}, ramal:'', caminho:'',  }, assessoria:{_id:'', sigla:'', descricao:'', interna:true, ordem:0} }; // Limpar o objeto
...
export class VideoConferenciaFormComponent implements OnInit {
  ...
  private isUsuarioLogado(): boolean {
    // Verifica se tem token usando o método existente do seu LoginService
    // Se o LoginService não tiver o método público, verificamos o localStorage direto
    const token = this.loginService.hasToken(); 
    if (!token) {
      this.snackBar.open('Acesso Negado: Você precisa estar logado para agendar ou editar.', 'Entendi', {
        duration: 4000,
        panelClass: ['warning-snackbar'] // Opcional: classe CSS para estilizar
      });
      return false;
    }
    return true;
  ...
  openVideoConferenciaModal(day: DateTime, videoConferencia?: VideoConferencia): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        if (videoConferencia) {
          Object.assign(videoConferencia, result);
          this.videoConferenciasService.save(videoConferencia).subscribe(() => this.refreshCalendar());
        } else {
          this.videoConferenciasService.save(result).subscribe(() => this.refreshCalendar());
        }
      }
    });
  }
